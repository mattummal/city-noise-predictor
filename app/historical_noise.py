...
def historical_noise_content():
	st.title('✨ Exploring the History of Noise Data')
	...
	with tab2:
		...
		# Group by option
		group_by_option = st.selectbox('Group By', ['By Hour', 'By Day', 'By Month', 'By Date'])
		...
		selected_data_40['result_timestamp'] = pd.to_datetime(selected_data_40['result_timestamp'])

		# Create additional columns
		selected_data_40['weekday'] = selected_data_40['result_timestamp'].dt.strftime('%A')
		...
		selected_laf_column = st.multiselect('Select variables', laf_columns, default=laf_columns)
		...
		# Plot the line graph
		fig = px.line(sorted_data, x=x_column, y=selected_laf_column, line_shape='linear')
		fig.update_layout(
		...
		selected_data_42 = file42[file42['location'] == list(file42.location.unique())[index]]
		...
		with st.expander('Definition of noise level measurements 👉'):
			...
			st.write('- `LAmax`: A-weighted, maximum sound level - maximum is not peak - dB(A)')
			st.write('- `LAeq`: A-weighted, equivalent continuous sound level - dB(C)')
			st.write('- `LCeq`: C-weighted, Leq (equivalent continuous sound level) - dB(C)')
		...
		# Plot the line graph
		fig42 = px.line(sorted_data_42, x=x_column, y=selected_l_column, line_shape='linear')
		fig42.update_layout(
		...
		df_file41 = df_file41[['result_timestamp', 'noise_event_laeq_primary_detected_class']]
		...
		# Aggregate the data
		aggregated_file41 = (
			df_file41.groupby(['hour', 'weekday', 'noise_event_laeq_primary_detected_class'])
			.size()
		...
		noise_types = list(aggregated_file41.noise_event_laeq_primary_detected_class.unique())
		# Select box for noise type
		selected_noise_type = st.selectbox('Select Noise Type', list(noise_types), index=2)
		# Filter the data for the selected noise only
		selected_aggregated_file41 = aggregated_file41[
			aggregated_file41['noise_event_laeq_primary_detected_class'] == selected_noise_type
	...
	with tab1:
		...
		# Noise event
		group_data = (
			file41.groupby([group_options[group_by][0], 'noise_event'])['noise_event_certainty']
			.count()
			.reset_index(name='count')
		)
		...
		if group_by == 'Date':
			fig.update_layout(xaxis=dict(tickformat='%b', dtick='M1', ticklabelmode='period'))
        ...
        - New York tends to be quieter during the weekends, which is not surprising!\
            Watch out for Thursday tho, when noise events hit their peak in New York.
        ...
        - New York takes a break from noise during the summer (July and August), as if the city is on vacation.
