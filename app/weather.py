...
def weather_content():
	st.title('☔️ Quality of Weather & Air')
	...
	with tab1:
		...
		with today_weather_col:
			...
			metric_style = (
				'padding: 20px; background-color: #f5f5f5; border-radius: 15px; text-align: left;'
			)
		...
		selected_hourly_weather = selected_hourly_weather.rename(columns=weather_column_names)
		...
		weather_fig = px.line(selected_hourly_weather, x='time', y=selected_weather_variables)
		...
		selected_hourly_air_vars = selected_hourly_air[['time'] + selected_air_variables]
	...
	with tab2:
		...
		weather_df = pd.read_csv('../data/processed_weather_data_leuven.csv', delimiter=',')
		...
		file42 = file42_0[file42_0['location'] == list(file42_0.location.unique())[index]]
