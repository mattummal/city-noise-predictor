...
def scrape_weather_air_data():
	...
	hourly_weather['precipitation'] = hourly_weather[['rain', 'showers', 'snowfall']].sum(axis=1)
	hourly_weather.drop(
		[
			'showers',
...
# load model
encoder_file42 = load_pickle_file('../model/model_noise_level_file42/encoder_model_file42.pkl')
...
model_laf25 = load_pickle_file('../model/model_noise_level_file40/xgb_laf25_per_hour.pkl')
model_laf50 = load_pickle_file('../model/model_noise_level_file40/xgb_laf50_per_hour.pkl')
model_laf75 = load_pickle_file('../model/model_noise_level_file40/xgb_laf75_per_hour.pkl')


def prediction_noise_content():
	st.title('💥 Predicting the Noise')
	st.write(
	...
	with st.expander('Definition of weather variables 👉'):
		...
		st.write('- `relativehumidity_2m`: Relative humidity at 2 meters above ground (%)')
		...
		st.write('- `pressure_msl`: Atmospheric air pressure reduced to mean sea level (hPa)')
		...
		st.write('- `vapor_pressure_deficit`: Vapor Pressure Deficit (VPD) in kilopascal (kPa)')
		st.write('- `precipitation`: Total precipitation sum of the preceding hour (mm)')
		st.write('- `snowfall`: Snowfall amount of the preceding hour in centimeters (cm)')
		st.write(
		...
		st.write('- carbon_monoxide: Carbon monoxide concentration close to surface (μg/m³)')
		st.write('- nitrogen_dioxide: Nitrogen dioxide concentration close to surface (μg/m³)')
		st.write('- sulphur_dioxide: Sulphur dioxide concentration close to surface (μg/m³)')
	...
	map_center = (50.8798, 4.7005)  # Center of NY
	map_zoom = 15
